...
def plot_data(x, y):
    # plot class 0
    plt.scatter(x.loc[y == 0, x.columns[0]], x.loc[y == 0, x.columns[1]], label='CLASS 0', alpha=0.5, linewidth=0.15)
    # plot class 1
    plt.scatter(x.loc[y == 1, x.columns[0]], x.loc[y == 1, x.columns[1]], label='CLASS 1', alpha=0.5, linewidth=0.15, c='r')
    # add legend
    plt.legend()
    # show plot
    plt.show()
...
def compare_plot(X, y, X_resampled, y_resampled):
    # Plot the original data using Pandas .loc indexing
    plt.subplot(1, 2, 1)
    plt.scatter(X.loc[y == 0, X.columns[0]], X.loc[y == 0, X.columns[1]], label="Class #0", alpha=0.5, linewidth=0.15)
    plt.scatter(X.loc[y == 1, X.columns[0]], X.loc[y == 1, X.columns[1]], label="Class #1", alpha=0.5, linewidth=0.15, c='r')
    ...
    # Plot the resampled data using Pandas .loc indexing
    plt.subplot(1, 2, 2)
    plt.scatter(X_resampled.loc[y_resampled == 0, X_resampled.columns[0]], X_resampled.loc[y_resampled == 0, X_resampled.columns[1]], label="Class #0", alpha=0.5, linewidth=0.15)
    plt.scatter(X_resampled.loc[y_resampled == 1, X_resampled.columns[0]], X_resampled.loc[y_resampled == 1, X_resampled.columns[1]], label="Class #1", alpha=0.5, linewidth=0.15, c='r')
    plt.title('Resampled Set')
    plt.legend()
...
##############################################################################################################
##############################################################################################################
st.header('BUSINESS RECOMMENDATIONS', divider='')
st.write("""
- With a recall of 85.7%, about 14.3% of fraudulent transactions are missed. These undetected frauds could lead to 
significant financial losses. We can consider **additional layers of review** or investigation for transactions that are 
flagged as borderline cases (high probability but below the threshold).
